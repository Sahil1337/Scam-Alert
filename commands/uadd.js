...
module.exports.run = async(client, message) => {
    ...
    message.channel.send('<:Green:765920149071986728>  **Please Tell The Scammer ID**');
    let collector = new discord.MessageCollector(message.channel, filter, { max: 1 });
    collector.on('collect', async(message, col) => {
        ID = message.content;
        if(isNaN(ID))return message.channel.send(`**Please Provide A Valid ID**`)
        let FindInIDs = await Scam.findOne({ ScammerID: ID })
        if(FindInIDs)return message.channel.send(`**Kiddo Already On ScamList**`)
        collector.stop()

    message.channel.send('<:Green:765920149071986728> **Please Tell The Scammer UserName**')
    let collector2 = new discord.MessageCollector(message.channel, filter, { max: 1 });
    collector2.on('collect', (message, col) => {
        Name = message.content;
        collector2.stop()

     message.channel.send('<:Green:765920149071986728> **Please Tell The Scammer Server Name , ID**')
        ...
        collector3.on('collect', (message, col) => {
            ...
            message.channel.send('<:Green:765920149071986728> **Please Explain How The Scam Happend**')
            let collector4 = new discord.MessageCollector(message.channel, filter, { max: 1 });
            collector4.on('collect', (message, col) => {
                reason = message.content;
                collector4.stop()

                message.channel.send('<:Green:765920149071986728> **Can You provide Some Screen Shots ( Links Of Screen Shot )**')
                let collector5 = new discord.MessageCollector(message.channel, filter, { max: 1 });
                collector5.on('collect', (message, col) => {
                    attachments = message.content;
                    if(!attachments.startsWith('https://'))return message.channel.send(`**This Was Not A Correct Link Of AttachMent Please Run The Command Again** `)
                    collector5.stop()

                    message.channel.send('<:Green:765920149071986728> **Okay Ty !! For Reporting A Scammer , Can You Tell Me The UserName Of The Kind Reporter**')
                    let collector6 = new discord.MessageCollector(message.channel, filter, { max: 1 });
